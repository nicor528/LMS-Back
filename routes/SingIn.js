...
router.post("/edit-info-user", async (req, res) => {
    const { user_ID, name, lastName, birth, postal_code, city, province, country, street_name, academic, phoneNumber } = req.body;
    const token = req.headers['authorization'] && req.headers['authorization'].split(' ')[1];
    const refreshToken = req.headers['refresh-token'];

    if (!user_ID || !name || !lastName || !birth || !postal_code || !city || !province || !country || !street_name || !academic || phoneNumber || !token) {
        return res.status(401).send({ message: "Missing data in the body", status: false });
    }
    ...
    try {
        ...
        // Si el token es válido, proceder con la lógica normal
        const user = await getUser2(user_ID);
        const updatedUser = await editInfoUser(user.id, name, lastName, birth, postal_code, city, province, street_name, academic, country, phoneNumber);
